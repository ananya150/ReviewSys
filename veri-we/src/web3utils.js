const { TransactionReceipt, TransactionResponse } = require('@ethersproject/abstract-provider');
const { BigNumberish, ContractReceipt } = require('ethers');
...
const UNIT = ethers.BigNumber.from(10).pow(18);
...
//  async function getTxTimestamp(txResponse: TransactionResponse) {
//   const result: TransactionReceipt = await txResponse.wait();
//   return (await ethers.provider.getBlock(result.blockNumber)).timestamp;
// }

//  function getEvent(receipt: ContractReceipt, eventName: string): TypedEvent {
//   if (!receipt.events) {
...
//  function getAllMatchingEvents(receipt: ContractReceipt, eventNames: string[]): TypedEvent[] {
//   if (!receipt.events) {
//     throw Error('no events on contract receipt');
//   }
...
//  function getEventArgs(receipt: ContractReceipt, eventName: string) {
//   return getEvent(receipt, eventName).args;
// }

// // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
//  function getOptionTypeName(optionType: OptionType) {
//   switch (optionType) {
